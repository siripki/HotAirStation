...
void loop() {
  ...
  if (temp != prevTemp || setHeatLCD != prevSetHeatLCD || setFanLCD != prevSetFanLCD || reedStat != prevReedStat) { //melakukan update layar hanya jika ada perubahan nilai pada variabel
    display();
    prevTemp = temp;
    prevSetHeatLCD = setHeatLCD;
    prevSetFanLCD = setFanLCD;
    prevReedStat = reedStat;
  }
}
void display() {
    lcd.setCursor(0,0);
